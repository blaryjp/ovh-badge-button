;(function (factory) {
    ...
    function setConsumerKey (ck) {
        keys.ck = ck;
        sessionStorage.setItem('ovh-ck', ck);
    }
    ...
    keys.ck = sessionStorage.getItem('ovh-ck');
    ...
    function login (urlToRedirect) {
        ...
        if (isLogged()) {
            sessionStorage.removeItem('ovh-ck');
            keys.ck = null;
        }
        ...
        return $.ajax({
            ...
            sessionStorage.setItem('ovh-ck', keys.ck);

            // Redirect to Auth page
            return window.open(data.validationUrl);
...
(function () {
  var $app = $("#demo");
  var $btn = $app.find(".btn-dash");
  var $success = $app.find(".success-wrapper");
  ...
  $(document).ready(function () {
    $app.on("click", ".btn-dash", function () {
      order();
    });
  });
})();
...
(function () {
  ...
  var $btn = $app.find(".btn-dash");
  var $success = $app.find(".success-wrapper");
  var $loading = $app.find(".loading");
  var $loginBtn = $app.find(".login > button");
...
/*$.ovh.login().then(function (url) {
  ...
  function order () {
    $btn.prop("disabled", true);
    debug("[ORDER] Let's go!");
    return $.ovh.post("/order/cart", {
      data: {
        ovhSubsidiary: user.ovhSubsidiary
      }
    }).done(function (cart) {
      debug("Cart created: " + cart.cartId);
      return $.ovh.post("/order/cart/{cartId}/assign", {
        params: {
          cartId: cart.cartId
        }
      }).done(function () {
        debug("Cart assigned!");
        return $.ovh.post("/order/cart/{cartId}/dbaasTimeseries", {
          params: {
            cartId: cart.cartId
          },
          data: {
            duration: "P1M",
            planCode: "payg_account",
            pricingMode: "default",
            quantity: 1
          }
        }).done(function () {
          debug("Plancode payg_account POSTed!");
          return $.ovh.post("/order/cart/{cartId}/checkout", {
            params: {
              cartId: cart.cartId
            }
          }).done(function (result) {
            debug("Checkout done!");
            debug("BC URL: " + result.url);
            showSuccess();
          });
        });
      });
  ...
  function init () {
    $.ovh.get("/me").done(function (me) {
      user = me;
      $loading.hide();
      $app.find(".ovh-dash").show();
      debug("Logged: " + me.nichandle);
    }).fail(function () {
      $loading.hide();
      $app.find(".login").show();
      debug("Not logged");
